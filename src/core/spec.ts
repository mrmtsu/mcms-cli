...
const GLOBAL_OPTIONS: GlobalOptionSpec[] = [
  ...
  { name: "--profile <name>", description: "profile name to resolve service domain / keychain key" },
  ...
  { name: "--retry-max-delay <ms>", description: "max retry delay in milliseconds (default: 3000)" },
...
const COMMANDS: CommandSpec[] = [
  {
    ...
    args: [],
    options: ["--profile <name>", "--service-domain <serviceDomain>", "--api-key <apiKey>", "--api-key-stdin", "--prompt"],
    readOnly: false
  },
  {
    path: "auth status",
  ...
  { path: "api info", description: "show API details", args: ["<endpoint>"], options: [], readOnly: true },
  { path: "config doctor", description: "show resolved auth/config sources and common risks", args: [], options: [], readOnly: true },
  {
  ...
  { path: "completion uninstall", description: "remove installed completion scripts", args: [], options: [], readOnly: false },
  { path: "completion endpoints", description: "print endpoint candidates for shell completion", args: [], options: [], readOnly: true },
  {
    path: "content list",
    description: "list content",
    args: ["<endpoint>"],
    options: ["--limit <limit>", "--offset <offset>", "--orders <orders>", "--q <q>", "--filters <filters>", "--fields <fields>", "--ids <ids>", "--depth <depth>", "--draft-key <draftKey>", "--all"],
    readOnly: true
  },
  {
    path: "content get",
